...
import {emitAnalytic, getWaitlistURL} from "@/lib/futils";
export function CTA() {
        ...
        <section className="h-[25rem] md:h-[40rem] w-full bg-black bg-grid-white/[0.2]  relative flex items-center justify-center">
            ...
            <div className="flex flex-col items-center justify-center h-[40rem] md:max-w-64 ">
                ...
                <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 space-x-0 md:space-x-4 pt-1">
                    <Link className="relative inline-flex sm:h-16 md:h-12 overflow-hidden rounded-full p-[1px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50"
                          href={getWaitlistURL()}
                          onClick={() => { emitAnalytic({ name: "Waitlist", value: "CTA", data: {page: "main"}}) }}
                    >
